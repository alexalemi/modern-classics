...
| The First Six Books of the Elements of Euclid (1847) | Oliver Byrne | PD (London: Pickering, 1847). Scans: firstsixbooksofe00byrn, firstsixbooksel00byrn, firstsixbooksofe00eucl (all ~310 leaves). RESTORED, and the plates ARE the book: every proof is written in coloured diagram fragments set inline in the sentence, so it is the calculus-made-easy problem with colour. Redraw as SVG (the kenzeiki/game-of-logic rule: what was set is set again), and read every page against the scan. Check whether an existing faithful public-domain or openly licensed digital redraw already exists before starting. Added by Alex 2026-09-20 |
