"""Per-book checks for mill/ — the euclid-rivals pattern.
...
# KEPT DELIBERATELY, and by EXACT PHRASE rather than by loosening the
# sweep (the grimm rule). Chapter Two's account of Christian morality
# turns on the grammatical form of the commandments -- '"thou shalt
# not" predominates unduly over "thou shalt"' -- so the archaism there
# is the thing being described and modernising it would delete the
# observation. Nothing else in the book needs an exemption: the only
# other hit in the source, "whereupon" in Chapter Five, is ordinary
# Mill connective prose and should not survive translation.
THOU_OK = ["thou shalt"]
